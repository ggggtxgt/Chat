...
#include "../forms/ui_AuthenFriend.h"
...
AuthenFriend::AuthenFriend(QWidget *parent) : QDialog(parent), ui(new Ui::AuthenFriend), _label_point(2, 6) {
    ...
    ui->back_ed->setPlaceholderText("lyy");
...
void AuthenFriend::addLabel(QString name) {
    ...
    connect(tmplabel, &FriendLabel::signal_close, this, &AuthenFriend::SlotRemoveFriendLabel);
...
void AuthenFriend::SlotApplySure() {
    ...
    // 发送tcp请求给chat server
    emit TcpManager::GetInstance()->signal_send_data(RequestId::ID_AUTH_FRIEND_REQ, jsonData);
