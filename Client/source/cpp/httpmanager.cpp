...
void HttpManager::PostHttpRequest(QUrl url, QJsonObject json, RequestId id, Moudles moudle) {
    ...
    QObject::connect(reply, &QNetworkReply::finished, [self, reply, id, moudle] {
        // 处理错误情况
        if (reply->error() != QNetworkReply::NoError) {
            qDebug() << "PostHttpRequest Error: " << reply->errorString();

            // 即使错误，也应通知其他模块发送完毕
            emit self->signal_http_finish(id, "", ErrorCodes::ERR_NETWORK, moudle);
        ...
        // 发送成功，通知其他模块发送完毕
        emit self->signal_http_finish(id, res, ErrorCodes::SUCCESS, moudle);
        reply->deleteLater();
        return;
    });
}
