...
MysqlConnPool::MysqlConnPool(const std::string &url, const std::string &user,
    ...
    try {
        for (int i = 0; i < size_; i++) {
            ...
            sqlpool_.push(std::make_unique<SqlConnection>(con, timestamp));
        }
        ...
        LOG(ERROR) << "mysql pool init failed is: " << exception.what();
    }
}

void MysqlConnPool::CheckConnection() {
    std::lock_guard<std::mutex> guard(mutex_);
    int size = sqlpool_.size();
    // 获取当前时间戳
    auto currentTime = std::chrono::system_clock::now().time_since_epoch();
    // 将时间戳转换为秒数
    long long timestamp = std::chrono::duration_cast<std::chrono::seconds>(currentTime).count();
    for (int i = 0; i < size; i++) {
        auto con = std::move(sqlpool_.front());
        sqlpool_.pop();
        Defer defer([this, &con]() {
            sqlpool_.push(std::move(con));
        });
...
std::unique_ptr<SqlConnection> MysqlConnPool::GetConnection() {
    ...
    cond_.wait(lock, [this] {
        ...
        return !sqlpool_.empty();
    });

    if (b_stop_) {
        return nullptr;
    }
    std::unique_ptr<SqlConnection> con(std::move(sqlpool_.front()));
    sqlpool_.pop();
    return con;
}
...
MysqlDao::MysqlDao() {
    ...
    conpool_.reset(new MysqlConnPool(host + ":" + port, user, pwd, schema, 5));
}
...
bool MysqlDao::AddFriend(const int &from, const int &to, std::string back_name) {
    ...
    try {
        ...
        con->_con->commit();
        std::cout << "addfriend insert friends success";
