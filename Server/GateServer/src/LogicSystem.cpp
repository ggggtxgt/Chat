...
LogicSystem::LogicSystem() {
    ...
    RegisterPost("/user_register", [](std::shared_ptr<HttpConnection> conn) {
        ...
        if (!b_get_varify) {
            LOG(INFO) << " get varify code expired";
    ...
    RegisterPost("/reset_pwd", [](std::shared_ptr<HttpConnection> connection) {
        ...
        bool b_get_varify = RedisManager::GetInstance()->Get(CODEPREFIX + src_root["email"].asString(), varify_code);
        if (!b_get_varify) {
            LOG(ERROR) << " get varify code expired";
            root["error"] = ErrorCodes::VarifyExpired;
            std::string jsonstr = root.toStyledString();
            beast::ostream(connection->_response.body()) << jsonstr;
            return true;
        ...
        if (!email_valid) {
            LOG(ERROR) << " user email not match";
        ...
        LOG(INFO) << "succeed to update password" << pwd;
        int uid = MysqlManager::GetInstance()->GetUserIdByName(name);
        root["error"] = 0;
        root["uid"] = uid;
        root["email"] = email;
        root["user"] = name;
